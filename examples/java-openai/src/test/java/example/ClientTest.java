...
class ClientTest {
    @Test void errorsAndRedirectsNeverReplayInference() throws Exception {
        for (int status : new int[]{200, 307, 429, 503}) {
            ...
            try {
                ...
                if (status == 200) assertEquals("example", client.chat().completions().create(request).id());
                else assertThrows(Exception.class, () -> client.chat().completions().create(request));
